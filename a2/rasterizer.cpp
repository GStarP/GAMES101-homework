...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    int bb_min_x = (int)std::floor(min_x);
    int bb_max_x = (int)std::ceil(max_x);
    int bb_min_y = (int)std::floor(min_y);
    int bb_max_y = (int)std::ceil(max_y);
    ...
    for (int x = bb_min_x; x < bb_max_x; x++) {
        for (int y = bb_min_y; y < bb_max_y; y++) {
            ...
            if (insideTriangle(center_x, center_y, t.v)) {
                ...
                // 如果 z 插值小于当前像素的缓存深度, 则进行替换
                if (z_interpolated < depth_buf[get_index(x, y)]) {
                    depth_buf[get_index(x, y)] = z_interpolated;
                    Vector3f px((float)x, (float)y, 1);
                    set_pixel(px, t.getColor());
                }
            }
        }
    }
